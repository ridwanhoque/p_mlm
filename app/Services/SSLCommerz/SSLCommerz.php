...
class SSLCommerz
{
    ...
    {
        $this->setSSLCommerzMode(config('services.sslcommerz.is_sandbox'));
        $this->store_id = config('services.sslcommerz.store_id');
        $this->store_pass = config('services.sslcommerz.store_password');
...
//                            header('Location: ' . $this->sslc_data['GatewayPageURL']);
                            return redirect($this->sslc_data['GatewayPageURL']);
    ...
    protected function sendRequest($data)
    {
        ...
        if (config('services.sslcommerz.is_localhost')) {
    ...
    public function ipnHashVerify(Request $request) {

        if($request->filled('verify_sign', 'verify_key')) {
            # NEW ARRAY DECLARED TO TAKE VALUE OF ALL POST
            ...
            # ADD MD5 OF STORE PASSWORD
            $new_data['store_passwd'] = md5($this->store_pass);
